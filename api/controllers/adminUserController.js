...
export const createAdminUser = async (req, res, next) => {
  try {
    ...
    await logAudit({
      ...
      action: "CREATE_ADMIN", entityType: "admin_user", entityId: String(record.id),
      newValue: { name, role, created_by_role: req.user?.role },
    });
...
export const updateAdminUser = async (req, res, next) => {
  try {
    ...
    await logAudit({
      userId: req.user?.id, userName: req.user?.name ?? "system",
      action: "UPDATE_ADMIN", entityType: "admin_user", entityId: String(id),
      newValue: { name, role, status, updated_by_role: req.user?.role },
    });
...
export const deleteAdminUser = async (req, res, next) => {
  try {
    ...
    await logAudit({
      userId: req.user?.id, userName: req.user?.name ?? "system",
      action: "DELETE_ADMIN", entityType: "admin_user", entityId: String(id),
      oldValue: { name: record.name, role: record.role, deleted_by_role: req.user?.role },
    });
