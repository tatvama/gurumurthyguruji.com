...
const TrikalaReading = {
  async create({ case_reference, full_name, mobile, whatsapp, email, gender, occupation, city, district, state, pincode, dob, tob, pob, service_type, guidance_query, palm_image, priority, preferred_language, devotee_id, consent }) {
    const { rows } = await pool.query(
      `INSERT INTO trikala_readings
         (case_reference, full_name, mobile, whatsapp, email, gender, occupation, city, district, state, pincode, dob, tob, pob,
          service_type, guidance_query, palm_image, priority, preferred_language, devotee_id, consent)
      ...
      [case_reference, full_name, mobile, whatsapp || null, email, gender, occupation, city || null,
       ...
       priority || "Normal", preferred_language || null, devotee_id || null, consent || false]
    );
    return rows[0];
  },
